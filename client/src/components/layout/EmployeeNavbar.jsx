...
const EmployeeNavbar = ({ toggleSidebar }) => {
  ...
  const initials = `${user.firstName?.[0] || "E"}${user.lastName?.[0] || ""}`;
    ...
    <header className="h-16 bg-white border-b border-gray-200 flex items-center px-5 gap-4 shrink-0 z-10">
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        <div className="relative group">
          ...
          {/* Dropdown */}
          <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-gray-200 rounded-xl shadow-lg py-1 hidden group-hover:block z-50">
            <div className="px-4 py-2.5 border-b border-gray-100">
              <p className="text-sm font-semibold text-gray-800">{user.firstName} {user.lastName}</p>
              <p className="text-xs text-gray-400 truncate">{user.email || "employee@example.com"}</p>
            </div>
            <button
